...
| CL-8 | UI/UX Design | Started the Figma low-fi/hi-fi prototype (`BugTrack — UI/UX Design`, https://www.figma.com/design/HdHr20k8fhxawBYqn9AWxo) with AI-driven building via Figma's MCP tools. Completed the Cover page, all 8 low-fidelity wireframes, and a full reusable hi-fi component library (Button, Form Field, Badge, Flash Banner, Navbar — variant sets bound to the real app's actual Bootstrap/CSS palette read from `public/css/style.css`) plus 2 of 8 hi-fidelity screens (Register, Login) before the Figma account's Starter-plan tool-call cap (20 AI tool calls/month) was reached mid-build. Rather than pause the whole artefact on a billing constraint, the remaining 6 hi-fidelity screens (Reporter Dashboard × 2 states, Report a Bug × 2 states, Admin Dashboard, Manage Bug) were handed off as a written build spec so they could be finished manually in Figma's ordinary editor, reusing the already-built component library — no further AI generation involved for those 6 | An external platform quota, not a design or requirements problem — worth logging because it's a real constraint that shaped *how* an artefact was produced (partly AI-authored, partly human-assembled from AI-authored components), which is exactly the kind of thing a GenAI disclosure should be honest about rather than smoothing over | UI/UX Design artefact (Figma file); GenAI disclosure (Component 6) — this entry is itself part of that disclosure |
| CL-9 | Deployment prep | Closed the one real gap the State Diagram flagged as a planned enhancement: `routes/admin.js`'s `POST /admin/bug/:id` now rejects a status change that moves backward through `Open → In Progress → Resolved` (server-side, before any DB write), instead of silently accepting any enum value regardless of the bug's current status. Also added a proper `.env.example` to the real `trackbug` folder (placeholders only) — it had a correctly-gitignored real `.env` but, unlike `copy_trackbug`, no committed template, so a fresh clone had nothing to copy from | Picked as the live "make a change and propagate it" demonstration precisely because it was already documented as a gap rather than discovered fresh — the traceability runs System Design (Page 5) → this entry → the code itself, which is the point of keeping the log in the first place. The `.env.example` fix piggybacked on the same session for the same reason CL-7 exists: a missing template is a smaller version of the same class of problem | `routes/admin.js`; System Design Page 5 (state diagram gap now closed — see that document's own addendum); `.env.example` |
| CL-10 | Git Version Control Practice prep | Attempted to build the project's JIRA space (epics/stories, project key `TB`) directly via the Atlassian API to mirror the Epic A–F / BT-1–BT-14 backlog above. Blocked immediately: the connected Atlassian/Jira integration was authorised only for a different, unrelated site (`dost-qut-wil.atlassian.net`, project `DOST`) than the actual project site (`connect-team-ks37m8yg.atlassian.net`, project `TB`) — an authorisation-scope mismatch, not a data or design problem. Rather than block on re-authorising a third-party integration mid-session, the same Epic A–F / BT-1–BT-14 structure was handed off as a field-by-field build spec (issue type, summary, description, epic link, status, and a `TB-n` branch/commit naming convention) for manual entry into the already-created JIRA space | Same shape as CL-8 (Figma Starter-plan cap): an external platform/authorisation constraint, not a requirements or process gap, so it's logged rather than silently worked around. Keeps the backlog's single source of truth in this document — JIRA becomes a second *view* of the same BT-n IDs, not a divergent copy | Product Backlog (§6, this document); JIRA space (`TB` project); Git Version Control Practice evidence (branch/commit naming); GenAI disclosure (Component 6) |
| CL-11 | Git Version Control Practice, follow-up | Re-authorising the Atlassian integration surfaced a second constraint: `connect-team-ks37m8yg.atlassian.net` never appeared as a selectable site even after reconnecting under the correct account (`jal.panchal09122002@gmail.com`), consistent with an organisation-level app-access policy blocking third-party connectors on that site rather than a login problem. Rather than continue chasing access to a site outside this account's control, a fresh JIRA space was created directly (`BugTrack`, project key **`BT`** — deliberately chosen to match the `BT-n` IDs already used throughout this document, rather than the originally planned `TB`) on `jalpanchal09122002.atlassian.net`, a site this account does control. All 16 backlog items (`BT-1`–`BT-14` plus `BT-15`/`BT-16` for the CL-9 fixes) were entered manually from the CL-10 build spec, grouped under the 6 Epics (A–F), with the 6 cut items (`BT-6`, `BT-9`, `BT-10`, `BT-11`, `BT-12`, `BT-13`) set to `Done` with an explanatory comment rather than left ambiguous — this project's Team-managed Scrum workflow has no `Won't Do` status by default, so `Done` + comment is the closest available equivalent to the cut/struck-through convention used everywhere else in this document | The `TB` project key from CL-10's spec was never realised — worth recording so the discrepancy between the spec document and the live JIRA space (key `BT`, not `TB`) is explained rather than silently inconsistent. Also documents *why* a second, self-owned JIRA site was used instead of continuing to pursue access to the originally-linked one: an org policy outside this account's control, not abandoned effort | Product Backlog (§6, this document — `BT-n` IDs now literally match JIRA issue keys); JIRA space (`BugTrack`, project `BT`, `jalpanchal09122002.atlassian.net`); Git Version Control Practice evidence; GenAI disclosure (Component 6) |
